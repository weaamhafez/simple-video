	...
	$.fn.jAlert = function(options) {
		...
		var addAlert = function(content){
			...
			if( thisAlert.options.closeBtn ){
				...
				thisAlert.instance.click('.closejAlert', function(e){
					e.preventDefault();
					$(this).parents(".ja_wrap").children(".jAlert").closeAlert();
					return false;
				});
			...
			if( thisAlert.options.closeOnEsc ){
				...
				/* Unbind if already exists */
				$(document).undelegate('','keydown', $.fn.jAlert.onEscKeyDown);

				/* Bind keydown */
				$(document).delegate('','keydown', $.fn.jAlert.onEscKeyDown);
